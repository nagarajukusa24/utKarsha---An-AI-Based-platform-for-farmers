    ...
    switch ($_GET["action"]) {
        case "add":
            if (! empty($_POST["quantity"])) {
                ...
                $cartResult = $shoppingCart->getCartItemByProduct($productResult[0]["product_id"], $member_id);
                ...
                if (! empty($cartResult)) {
                    ...
                    // Add to cart table
                    $shoppingCart->addToCart($productResult[0]["product_id"], $_POST["quantity"], $member_id);
                }
            }
            break;
        case "remove":
        ...
        case "confirm":
            ...
            <?php 
            $shoppingCart->confirmOrder($member_id);
            header('location:./payment/form2.php');
                break;
    }
}
?>
    ...
    $.ajax({
		...
		success : function(response) {
            ...
            $("div[id*='cart-price-']").each(function() {
                var cart_price = $(this).text().replace("RS.","");
...
?>
    <div id="shopping-cart">
        <div class="txt-heading">
            <div class="txt-heading-label">Agent Cart</div>

            <a id="btnEmpty" href="index1.php?action=empty"><img
            ...
            <button id='b'><a href="index1.php?action=confirm">Confirm Order</a></button>
    ...
    foreach ($cartItem as $item) {
        ?>
                ...
                <div class="cart-info action">
                    <a
                        href="index1.php?action=remove&id=<?php echo $item["cart_id"]; ?>"
